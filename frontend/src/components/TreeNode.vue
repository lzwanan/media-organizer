    ...
    <div v-if="node.type === 'directory'" class="flex items-center gap-1">
      ...
      <button @click="expanded = !expanded"
        ...
        <div class="flex-1 min-w-0 text-left overflow-hidden">
          <span class="text-sm font-medium text-gray-700 dark:text-gray-300 truncate block" :title="node.name">{{ node.name }}</span>
          <div v-if="recInfo?.target_name && recInfo.target_name !== node.name" class="mt-0.5">
            <span class="text-xs text-indigo-500 dark:text-indigo-400 truncate block">→ {{ recInfo.target_name }}</span>
          </div>
        </div>
        <span class="text-xs text-gray-400 shrink-0 ml-1">{{ childCount(node.path) }} items</span>
      </button>
    </div>
    ...
    <div v-else class="flex items-center gap-1">
      ...
      <div class="flex-1 px-2 py-2 ml-0.5 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
        ...
        <div class="flex items-center gap-2">
          ...
          <span class="text-xs text-gray-500 dark:text-gray-500 truncate flex-1 font-mono min-w-0" :title="node.name">{{ node.name }}</span>
          <span v-if="recInfo" class="text-[10px] font-semibold px-1.5 py-0.5 rounded-full shrink-0"
        ...
        <div v-if="recInfo?.target_name" class="flex items-center gap-2 mt-1 pl-6">
          ...
          <span class="text-sm font-medium text-indigo-600 dark:text-indigo-400 truncate min-w-0" :title="recInfo.target_name">{{ recInfo.target_name }}</span>
        </div>
        <!-- Pending -->
        <div v-if="!recInfo" class="flex items-center gap-2 mt-1 pl-6">
...
defineEmits<{ "toggle-select": [path: string] }>();
...
function children(parentPath: string): FileNodeResponse[] { return props.tree[parentPath] ?? []; }
function childCount(parentPath: string): number { return children(parentPath).length; }
