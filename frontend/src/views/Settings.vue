<template>
  <div class="max-w-2xl mx-auto px-6 py-12">
    ...
    <SectionCard>
      <h2 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-5">{{ $t('settings.namingStyle') }}</h2>
      <div class="space-y-4">
        <div v-for="t in ['movie','tv','anime']" :key="t" class="flex items-center gap-4">
          ...
          <div class="flex gap-1">
            <button v-for="s in ['en','zh','bilingual']" :key="s" @click="naming[t] = s"
    ...
    <div class="mt-6">
      <SectionCard>
        <h2 class="text-sm font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-5">{{ $t('settings.apiKeys') }}</h2>
        <div class="space-y-4">
          <div>
            <label class="text-xs text-gray-500 dark:text-gray-400 mb-1.5 block">{{ $t('settings.tmdbKey') }}</label>
            <input v-model="config.tmdb_key" type="password" :placeholder="$t('settings.tmdbKey')"
          ...
          <div>
            <label class="text-xs text-gray-500 dark:text-gray-400 mb-1.5 block">{{ $t('settings.translateKey') }}</label>
            <input v-model="config.translate_key" type="password" :placeholder="$t('settings.translateKey')"
              class="w-full h-10 px-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 text-sm text-gray-900 dark:text-gray-100 outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/10 transition-all" />
          </div>
        </div>
      </SectionCard>
    ...
    <div class="mt-8 flex gap-3 justify-end">
      <button @click="resetDefaults" class="px-5 py-2.5 rounded-xl text-sm text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
        {{ $t('common.reset') }}
      </button>
      <button @click="save" class="px-6 py-2.5 rounded-xl bg-indigo-500 hover:bg-indigo-600 text-white text-sm font-semibold transition-colors">
        {{ $t('common.save') }}
      </button>
    </div>
  </div>
...
async function save() {
  try {
    ...
    toast.add({ severity: "success", summary: t('settings.saved'), detail: t('settings.settingsUpdated'), life: 2000 });
  } catch {
    toast.add({ severity: "error", summary: t('common.error'), detail: t('settings.saveFailed'), life: 3000 });
  }
}
...
async function resetDefaults() {
  try {
    ...
    toast.add({ severity: "success", summary: t('common.reset'), detail: t('settings.resetDone'), life: 2000 });
  } catch {}
}
</script>
